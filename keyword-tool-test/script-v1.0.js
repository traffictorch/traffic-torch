...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Button handlers for URL + Code input
  ...
  if (urlAnalyzeBtn) {
    urlAnalyzeBtn.addEventListener('click', async () => {
      ...
      if (!yourUrl || !phrase) {
        alert('Please enter both URL and target keyword');
      ...
      startSpinnerLoader();
      runKeywordAnalysis({ url: fullUrl, phrase, inputType: 'url', rawCode: null });
      hasCheckedLimit = false;
    });
  }
  ...
  if (codeAnalyzeBtn) {
    codeAnalyzeBtn.addEventListener('click', async () => {
      ...
      const phrase = targetKeywordInput.value.trim();
      if (!rawCode || !phrase) {
        alert('Please paste HTML code and enter target keyword');
        hasCheckedLimit = false;
        return;
      }
      startSpinnerLoader();
      runKeywordAnalysis({ url: null, phrase, inputType: 'code', rawCode });
      hasCheckedLimit = false;
    });
  }
  ...
  // === RESTORED ORIGINAL HELPER FUNCTIONS (exactly as in your first version) ===
  const countPhrase = (text, phrase, isUrl = false) => {
    ...
    if (isUrl) {
      const urlWords = cleanText.split(/[-_\s]+/);
      const matchedWords = new Set();
      phraseWords.forEach(word => {
        if (urlWords.includes(word)) matchedWords.add(word);
      });
      const required = Math.ceil(phraseWords.length / 2);
      return matchedWords.size >= required ? 1 : 0;
    }
    const regex = new RegExp('\\b' + cleanPhrase.replace(/[-_\s]+/g, '\\b|\\b') + '\\b', 'gi');
  ...
  const fetchPage = async (inputData) => {
    try {
      let fetchUrl = PROXY;
      if (inputData.inputType === 'code' && inputData.rawCode) {
        fetchUrl += '?html=' + encodeURIComponent(inputData.rawCode);
      } else if (inputData.url) {
        fetchUrl += '?url=' + encodeURIComponent(inputData.url);
      }
      const res = await fetch(fetchUrl);
      if (!res.ok) return null;
      const html = await res.text();
      if (html.trim().startsWith('{') && html.includes('"blocked": true')) {
        return { blocked: true };
      }
  ...
  async function runKeywordAnalysis(inputData) {
    const { url, phrase, inputType, rawCode } = inputData;
    const yourDocOrBlocked = await fetchPage(inputData);
    if (!yourDocOrBlocked) {
      stopSpinnerLoader();
      results.innerHTML = `<p class="text-red-500 text-center text-xl p-10">Error: Page not reachable.</p>`;
      return;
    }
    if (yourDocOrBlocked.blocked === true) {
      stopSpinnerLoader();
      results.innerHTML = `
        <div class="max-w-2xl mx-auto px-6 py-12 text-center">
          ...
          <p class="text-lg text-gray-700 dark:text-gray-300 mb-8">
            The page is protected by a security system. Use Code Analysis instead.
          </p>
    ...
    const yourDoc = yourDocOrBlocked;
    let yourScore = 0;
    const data = {};
    const allFixes = [];
    ...
    data.urlSchema = { urlMatch: countPhrase(url || '', phrase, true), schema: schemaPresent ? 1 : 0 };
    ...
    results.innerHTML = `
        ...
        <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-2xl p-6 sm:p-8 md:p-10 w-full max-w-sm sm:max-w-md border-4 ${yourScore >= 80 ? 'border-green-500' : yourScore >= 60 ? 'border-orange-400' : 'border-red-500'}">
          ...
          <div class="relative aspect-square w-full max-w-[240px] sm:max-w-[280px] mx-auto">
            <svg viewBox="0 0 200 200" class="w-full h-full transform -rotate-90">
              ...
              <circle cx="100" cy="100" r="90" stroke="${yourScore >= 80 ? '#22c55e' : yourScore >= 60 ? '#fb923c' : '#ef4444'}" stroke-width="16" fill="none" stroke-dasharray="${(yourScore / 100) * 565} 565" stroke-linecap="round"/>
            </svg>
            <div class="absolute inset-0 flex items-center justify-center">
              <div class="text-center">
                <div class="text-5xl sm:text-6xl font-black drop-shadow-lg" style="color: ${yourScore >= 80 ? '#22c55e' : yourScore >= 60 ? '#fb923c' : '#ef4444'};">${yourScore}</div>
                <div class="text-lg sm:text-xl opacity-80 -mt-1" style="color: ${yourScore >= 80 ? '#22c55e' : yourScore >= 60 ? '#fb923c' : '#ef4444'};">/100</div>
              </div>
            </div>
          </div>
          ${(() => {
          ...
          <div class="mt-6 text-center">
            <p class="text-5xl sm:text-6xl font-bold ${bigGrade.color} drop-shadow-lg">${bigGrade.emoji} ${bigGrade.grade}</p>
          </div>
        </div>
      </div>
      ...
      <div class="max-w-5xl mx-auto my-16 px-4">
        <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-2xl p-8">
          ...
          <p class="text-center text-sm text-gray-600 dark:text-gray-400 mt-6 hidden md:block">Visual overview of your page performance across 7 key SEO Intent factors</p>
        </div>
      </div>
      ...
      <div class="grid md:grid-cols-3 gap-8 my-16">
        ${[
          ...
          if (m.name === 'Meta Title & Desc') {
            ...
            details = `<div class="mt-4 text-left space-y-2 text-sm">${data.meta.titleMatch > 0 ? '✅' : '❌'} <span class="font-bold">Meta Title:</span><br><span class="text-gray-800 dark:text-gray-200">${truncate(data.meta.titleText || '(none)', 80)}</span><br>${data.meta.descMatch > 0 ? '✅' : '❌'} <span class="font-bold">Meta Description:</span><br><span class="text-gray-800 dark:text-gray-200">${truncate(data.meta.descText || '(none)', 80)}</span></div>`;
            fixEdu = data.meta.yourMatches === 0 ? `The target keyword "${phrase}" is missing from both your title and meta description. This is a critical missed opportunity because search engines heavily weigh these elements when determining relevance. Adding the keyword naturally improves visibility and click-through rates significantly.` : data.meta.titleMatch === 0 ? `Your meta title is missing the target keyword. The title tag is the strongest on-page ranking factor and appears prominently in search results. Including the keyword early in the title helps Google match user queries better.` : data.meta.descMatch === 0 ? `Your meta description lacks the keyword. While it doesn't directly affect rankings, it influences click-through rates from search results. A compelling description with the keyword encourages more clicks.` : '';
          } else if (m.name === 'H1 & Headings') {
            score = data.h1.match > 0 ? 100 : 0;
            details = `<div class="mt-4 text-left space-y-2 text-sm">${data.headingsData.length > 0 ? data.headingsData.map(h => `${h.match ? '✅' : ''} <span class="font-bold">${h.tag}:</span> <span class="text-gray-800 dark:text-gray-200">${truncate(h.text, 60)}</span>`).join('<br>') : '<span class="text-gray-800 dark:text-gray-200">No headings found</span>'}</div>`;
            fixEdu = data.h1.match === 0 ? `Your main H1 heading does not contain the target keyword. The H1 is the most important heading and tells search engines the primary topic of the page. Without the keyword here, Google may struggle to understand your page focus clearly.` : '';
          } else if (m.name === 'Content Density') {
            score = calculateContentScore(data.content);
            details = `<div class="mt-4 text-center space-y-2 text-sm"><p class="text-gray-800 dark:text-gray-200"><span class="font-bold">Word count:</span> ${data.content.words}</p><p class="text-gray-800 dark:text-gray-200"><span class="font-bold">Keyword mentions:</span> ${data.content.matches}</p><p class="text-gray-800 dark:text-gray-200"><span class="font-bold">Density:</span> ${data.content.density}% (ideal 1-2%)</p></div>`;
            fixEdu = (() => {
              let edu = '';
            ...
            details = `<div class="mt-4 text-left space-y-2 text-sm"><p class="text-gray-800 dark:text-gray-200 font-bold">Matching alts (${data.alts.phrase}/${data.alts.total} images):</p>${data.alts.matchingAlts.length > 0 ? data.alts.matchingAlts.map(alt => `✅ <span class="text-gray-800 dark:text-gray-200">${truncate(alt, 60)}</span>`).join('<br>') : '<span class="text-gray-800 dark:text-gray-200">None found</span>'}</div>`;
            fixEdu = data.alts.phrase === 0 && data.alts.total > 0 ? `None of your ${data.alts.total} images have the target keyword in alt text. This misses opportunities for accessibility, image search traffic, and additional relevance signals. Focus on key images like hero or product photos first.` : '';
          } else if (m.name === 'Anchor Text') {
            score = data.anchors.count > 0 ? 100 : 0;
            details = `<div class="mt-4 text-left space-y-2 text-sm"><p class="text-gray-800 dark:text-gray-200 font-bold">Matching anchors (${data.anchors.count} found):</p>${data.anchors.matchingAnchors.length > 0 ? data.anchors.matchingAnchors.map(a => `✅ <span class="text-gray-800 dark:text-gray-200">${truncate(a.text, 50)}</span> → ${truncate(a.href, 40)}`).join('<br>') : '<span class="text-gray-800 dark:text-gray-200">None found</span>'}</div>`;
            fixEdu = data.anchors.count === 0 ? `No internal links use the target keyword in anchor text. This misses a chance to strengthen topical authority flow across your site. Aim for 1-3 natural keyword anchors linking to relevant internal pages.` : '';
          } else if (m.name === 'URL & Schema') {
            score = Math.min(100, (data.urlSchema.urlMatch ? 50 : 0) + (data.urlSchema.schema ? 50 : 0));
            details = `<div class="mt-4 text-left space-y-2 text-sm">${data.urlSchema.urlMatch > 0 ? '✅' : '❌'} <span class="font-bold">Keyword in URL</span><br><span class="text-gray-800 dark:text-gray-200">${truncate(url || '', 80)}</span><br>${data.urlSchema.schema ? '✅' : '❌'} <span class="font-bold">Structured Data (Schema)</span></div>`;
            fixEdu = (() => {
              let edu = '';
          ...
          const diagnostics = getModuleDiagnostics({name: m.name}, data, phrase, url);
            ...
            <div class="text-center p-6 bg-white dark:bg-gray-900 rounded-2xl shadow-lg border-4 ${borderColor}">
              ...
              <div class="relative w-28 h-28 mx-auto">
                <svg width="112" height="112" viewBox="0 0 112 112" class="transform -rotate-90">
                  ...
                  <circle cx="56" cy="56" r="48" stroke="${score >= 80 ? '#22c55e' : score >= 60 ? '#eab308' : '#ef4444'}" stroke-width="12" fill="none" stroke-dasharray="${(score / 100) * 301} 301" stroke-linecap="round"/>
                </svg>
                <div class="absolute inset-0 flex items-center justify-center">
              ...
              <div class="mt-4">
                <div class="text-2xl font-bold ${grade.color}">${grade.emoji} ${grade.grade}</div>
              </div>
              ${details}
              <button onclick="this.parentElement.querySelector('.fixes-panel').classList.toggle('hidden')" class="mt-4 px-6 py-2 bg-red-600 text-white rounded-full hover:bg-red-700 text-sm">Show Fixes</button>
              <div class="fixes-panel hidden mt-6 p-6 bg-red-50 dark:bg-red-900/20 rounded-2xl border border-red-200 dark:border-red-800">
                ...
                <div class="space-y-4 text-left">
                  ${diagnostics.map(d => `<div class="flex items-start gap-3"><span class="text-xl mt-1">${d.status}</span><div><p class="font-medium text-gray-800 dark:text-gray-200">${d.issue}</p>${d.how ? `<p class="text-sm text-gray-600 dark:text-gray-400 mt-1">${d.how}</p>` : ''}</div></div>`).join('')}
                  ${!hasIssues ? '<p class="text-center text-green-600 dark:text-green-400 font-bold text-lg">🎉 This module is fully optimized!</p>' : ''}
                </div>
                <div class="text-center mt-8 pt-6 border-t border-red-200 dark:border-red-700"><a href="/keyword-tool/#${hashId}" class="text-orange-600 dark:text-orange-400 font-bold hover:underline">Learn more about ${m.name}</a></div>
              </div>
              <button onclick="this.nextElementSibling.classList.toggle('hidden')" class="mt-4 px-6 py-2 bg-orange-500 text-white rounded-full hover:bg-orange-600 text-sm">More Details</button>
              <div class="hidden mt-6 space-y-6 text-left text-sm">
                <div class="text-center mb-4"><a href="/keyword-tool/#${hashId}" class="text-orange-600 dark:text-orange-400 font-bold hover:underline">How ${m.name} is tested?</a></div>
                ...
                <div class="text-center mt-8 pt-6 border-t border-gray-300 dark:border-gray-700"><a href="/keyword-tool/#${hashId}" class="text-orange-600 dark:text-orange-400 font-bold hover:underline">Learn more about ${m.name}</a></div>
              </div>
            </div>
          `;
        }).join('')}
      ...
      <div class="my-16">
        ...
        ${topPriorityFixes.length ? `<div class="space-y-8 max-w-4xl mx-auto">${topPriorityFixes.map((fix, i) => {
          const isSecond = i > 0 && fix.module === topPriorityFixes[0].module;
          return `<div class="p-8 bg-white dark:bg-gray-900 rounded-2xl shadow-xl border-l-8 border-orange-500 flex gap-6"><div class="text-5xl font-black text-orange-600">${i+1}</div><div class="flex-1"><div class="flex items-center gap-3 mb-3"><span class="px-4 py-1 bg-orange-500 text-white rounded-full text-sm font-bold">${fix.module}</span>${isSecond ? '<span class="text-sm text-orange-600 dark:text-orange-400">(multiple issues in this module)</span>' : ''}</div><h4 class="text-2xl font-bold text-gray-800 dark:text-gray-200 mb-3">${fix.issue}</h4><p class="text-gray-800 dark:text-gray-200">${fix.how}</p></div></div>`;
        }).join('')}</div>` : '<p class="text-center text-green-500 text-2xl font-bold">Strong optimization — keep it up!</p>'}
      </div>
      ...
      <div class="max-w-6xl mx-auto my-20 grid md:grid-cols-2 gap-8">
        <div class="p-4 bg-white dark:bg-gray-900 rounded-3xl shadow-2xl border-4 border-orange-500/20">
          ...
          <div class="flex justify-center items-baseline gap-8 mb-10">
            <div class="text-center"><div class="px-4 py-4 bg-gray-100 dark:bg-gray-800 rounded-2xl text-2xl font-bold text-gray-600 dark:text-gray-400">${levels[currentLevel]}</div><p class="text-lg mt-3 text-gray-600 dark:text-gray-400">Current</p></div>
            <div class="text-5xl text-orange-500">→</div>
            <div class="text-center"><div class="px-4 py-4 bg-green-100 dark:bg-green-900/30 rounded-2xl text-2xl font-bold text-green-700 dark:text-green-300">${levels[projectedLevel]}</div><p class="text-lg mt-3 text-gray-600 dark:text-gray-400">Projected</p></div>
          </div>
          ${topPriorityFixes.length ? `<div class="space-y-4"><p class="text-center text-lg font-medium text-gray-700 dark:text-gray-300 mb-6">Top priority fixes & impact:</p>${topPriorityFixes.map((fix, i) => `<div class="p-5 bg-orange-50 dark:bg-orange-900/20 rounded-2xl border border-orange-200 dark:border-orange-800"><div class="flex items-start gap-4"><div class="flex-shrink-0 w-10 h-10 flex items-center justify-center rounded-full bg-orange-100 dark:bg-orange-900/40 text-xl font-bold text-orange-700 dark:text-orange-300">${i+1}</div><div class="flex-1 min-w-0"><p class="text-gray-900 dark:text-gray-100 font-medium leading-relaxed"><span class="font-bold text-orange-700 dark:text-orange-300">${fix.issue}</span><br class="sm:hidden">${fix.how}</p></div></div></div>`).join('')}</div>` : `<div class="text-center py-10"><p class="text-4xl mb-4">🎉 Excellent Optimization!</p><p class="text-xl text-gray-600 dark:text-gray-400">Your on-page SEO is strong. Focus on backlinks and content freshness for further gains.</p></div>`}
          <details class="mt-10"><summary class="cursor-pointer text-lg font-bold text-orange-600 dark:text-orange-400 hover:underline">How We Calculated This</summary><div class="mt-6 space-y-3 text-gray-700 dark:text-gray-300 text-sm leading-relaxed"><p>• Based on proven on-page factors that correlate with higher rankings in large-scale studies.</p><p>• Each critical fix typically improves relevance signals and user satisfaction.</p><p>• Projected level assumes all top fixes are implemented naturally.</p><p class="italic">Conservative estimate — actual improvement can be higher depending on competition.</p></div></details>
        </div>
        <div class="p-4 bg-gradient-to-br from-green-500 to-teal-600 text-white rounded-3xl shadow-2xl">
          ...
          <div class="space-y-8">
            <div class="flex items-center gap-6"><div class="text-5xl">🖱️</div><div class="flex-1"><p class="text-xl font-medium">Click-Through Rate (CTR)</p><div class="mt-2 w-full bg-white/30 rounded-full h-10 overflow-hidden"><div class="h-full rounded-full flex items-center justify-end pr-6 font-black text-lg transition-all duration-700" style="width: ${hasMetaOrContent ? 75 : 50}%; background-color: ${hasMetaOrContent ? '#86efac' : '#fca5a5'};">+${hasMetaOrContent ? '25–40' : '15–30'}%</div></div></div></div>
            <div class="flex items-center gap-6"><div class="text-5xl">📈</div><div class="flex-1"><p class="text-xl font-medium">Impressions</p><div class="mt-2 w-full bg-white/30 rounded-full h-10 overflow-hidden"><div class="h-full rounded-full flex items-center justify-end pr-6 font-black text-lg transition-all duration-700" style="width: ${topPriorityFixes.length * 20}%; background-color: ${topPriorityFixes.length >= 2 ? '#86efac' : topPriorityFixes.length === 1 ? '#fdba74' : '#fca5a5'};">+${topPriorityFixes.length * 15}–${topPriorityFixes.length * 30}%</div></div></div></div>
            <div class="flex items-center gap-6"><div class="text-5xl">📊</div><div class="flex-1"><p class="text-xl font-medium">Average Position</p><div class="mt-2 w-full bg-white/30 rounded-full h-10 overflow-hidden"><div class="h-full rounded-full flex items-center justify-end pr-6 font-black text-lg transition-all duration-700" style="width: ${hasMetaOrContent ? 70 : 55}%; background-color: ${hasMetaOrContent ? '#86efac' : '#fca5a5'};">↑ ${hasMetaOrContent ? '4–8' : '2–5'} spots</div></div></div></div>
            <div class="flex items-center gap-6"><div class="text-5xl">🚀</div><div class="flex-1"><p class="text-xl font-medium">Organic Traffic</p><div class="mt-2 w-full bg-white/30 rounded-full h-10 overflow-hidden"><div class="h-full rounded-full flex items-center justify-end pr-6 font-black text-lg transition-all duration-700" style="width: ${topPriorityFixes.length * 25}%; background-color: ${topPriorityFixes.length >= 2 ? '#86efac' : topPriorityFixes.length === 1 ? '#fdba74' : '#fca5a5'};">+${topPriorityFixes.length * 20}–${topPriorityFixes.length * 45}%</div></div></div></div>
          </div>
          <div class="mt-10 space-y-3 text-sm leading-relaxed text-white dark:text-gray-200"><p>Conservative estimates based on pages with similar optimization levels.</p><p>Track changes in Google Search Console (Impressions, CTR, Average Position).</p><p>Expect movement within 7–30 days after indexing.</p></div>
        </div>
      </div>
      ...
      <div class="text-center my-16 px-4">
        ...
        <div id="share-form-container" class="hidden max-w-2xl mx-auto mt-8">
          <form id="share-form" class="space-y-6 bg-white dark:bg-gray-800 p-8 rounded-3xl shadow-xl border border-orange-500/30">
            <div><label for="share-name" class="block text-sm font-medium mb-2 text-gray-800 dark:text-gray-200">Your Name</label><input id="share-name" type="text" required placeholder="Your name" class="w-full bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-2xl px-6 py-4 focus:outline-none focus:border-orange-500"></div>
            <div><label for="share-sender-email" class="block text-sm font-medium mb-2 text-gray-800 dark:text-gray-200">Your Email (for replies)</label><input id="share-sender-email" type="email" required placeholder="your@email.com" class="w-full bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-2xl px-6 py-4 focus:outline-none focus:border-orange-500"></div>
            <div><label for="share-email" class="block text-sm font-medium mb-2 text-gray-800 dark:text-gray-200">Recipient Email</label><input id="share-email" type="email" required class="w-full bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-2xl px-6 py-4 focus:outline-none focus:border-orange-500"></div>
            <div><label for="share-title" class="block text-sm font-medium mb-2 text-gray-800 dark:text-gray-200">Email Title</label><input id="share-title" type="text" required placeholder="Traffic Torch Keyword Report" class="w-full bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-2xl px-6 py-4 focus:outline-none focus:border-orange-500"></div>
            <div><label for="share-body" class="block text-sm font-medium mb-2 text-gray-800 dark:text-gray-200">Message</label><textarea id="share-body" required rows="5" class="w-full bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-3xl px-6 py-4 focus:outline-none focus:border-orange-500"></textarea></div>
        ...
        <div id="feedback-form-container" class="hidden max-w-2xl mx-auto mt-8">
          <div class="bg-white dark:bg-gray-800 p-8 rounded-3xl shadow-xl border border-blue-500/30">
            <p class="text-lg font-medium mb-6 text-gray-800 dark:text-gray-200">Feedback for Keyword Tool on <strong>${document.body.getAttribute('data-url') || 'the analyzed page'}</strong></p>
            <form id="feedback-form" class="space-y-6">
              <div><label for="feedback-rating" class="block text-sm font-medium mb-2 text-gray-800 dark:text-gray-200">Rating (optional)</label><div class="flex gap-3 text-3xl justify-center sm:justify-start"><button type="button" data-rating="1" class="hover:scale-125 transition">😞</button><button type="button" data-rating="2" class="hover:scale-125 transition">🙁</button><button type="button" data-rating="3" class="hover:scale-125 transition">😐</button><button type="button" data-rating="4" class="hover:scale-125 transition">🙂</button><button type="button" data-rating="5" class="hover:scale-125 transition">😍</button></div><input type="hidden" id="feedback-rating" name="rating"></div>
              <div><label class="flex items-center gap-2 justify-center sm:justify-start"><input type="checkbox" id="reply-requested" class="w-5 h-5"><span class="text-sm font-medium text-gray-800 dark:text-gray-200">Request reply</span></label></div>
              <div id="email-group" class="hidden"><label for="feedback-email" class="block text-sm font-medium mb-2 text-gray-800 dark:text-gray-200">Your Email</label><input id="feedback-email" type="email" name="email" placeholder="your@email.com" class="w-full bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-2xl px-6 py-4 focus:outline-none focus:border-blue-500"></div>
              <div><label for="feedback-text" class="block text-sm font-medium mb-2 text-gray-800 dark:text-gray-200">Your Feedback</label><textarea id="feedback-text" name="message" required rows="5" maxlength="1000" class="w-full bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-3xl px-6 py-4 focus:outline-none focus:border-blue-500"></textarea><p class="text-xs text-gray-500 dark:text-gray-400 mt-1 text-center sm:text-left"><span id="char-count">0</span>/1000 characters</p></div>
    ...
    const failedMetrics = [];
    if (data.meta.titleMatch === 0 || !data.meta.titleText || data.meta.titleText.trim() === '') failedMetrics.push({ name: "Meta Title", grade: { text: "Needs Work", color: "text-red-600", emoji: "❌" } });
    if (data.meta.descMatch === 0 || !data.meta.descText || data.meta.descText.trim() === '') failedMetrics.push({ name: "Meta Description", grade: { text: "Needs Work", color: "text-red-600", emoji: "❌" } });
    if (data.urlSchema.schema === 0) failedMetrics.push({ name: "Structured Data (Schema)", grade: { text: "Needs Work", color: "text-red-600", emoji: "❌" } });
    if (data.alts.phrase === 0 && data.alts.total > 0) failedMetrics.push({ name: "Image Alts", grade: { text: "Needs Work", color: "text-red-600", emoji: "❌" } });
    if (failedMetrics.length > 0) renderPluginSolutions(failedMetrics);
    setTimeout(() => {
  ...
  // Original form listener (kept 100% unchanged for compatibility)
