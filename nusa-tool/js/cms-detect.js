/* cms-detect.js — shared Traffic Torch CMS / framework detector.
   Verbatim copy of the shared module. See /cms-detect.js for source of truth. */

const MAX_HTML_SCAN = 400_000;
const MIN_REPORT_SCORE = 3;
const TIE_MARGIN = 2;
...
function normalizeInput(input, options) {
  const out = { doc: null, html: '', url: (options && options.url) || '', mode: 'empty' };
  try {
...
function scoreOne(sig, ctx) {
  ...
  for (const s of (sig.signals || [])) {
    let matched = false;
    try {
      if (s.kind === 'html' && ctx.html) matched = s.re.test(ctx.html);
      else if (s.kind === 'meta' && ctx.meta) matched = s.re.test(ctx.meta);
      else if (s.kind === 'url' && ctx.url) matched = s.re.test(ctx.url);
      else if (s.kind === 'selector' && ctx.doc) matched = !!safeQuerySelector(ctx.doc, s.sel);
      else if (s.kind === 'text' && ctx.text) matched = s.re.test(ctx.text);
    } catch (_) { matched = false; }
    if (matched) { score += s.w; hits.push(s.label || s.kind); }
  }
  return { score, hits };
}
...
function confidenceFor(score) {
  for (const tier of CONFIDENCE_TIERS) if (score >= tier.min) return tier.level;
  return 'unknown';
}

export function detectCMS(input, options = {}) {
  const unknownResult = (mode) => ({
    id: 'unknown', name: 'Custom / Unknown', version: null,
    confidence: 'unknown', score: 0,
    signals: [], related: [], alternates: [], mode: mode || 'empty'
  });

  try {
    const ctx = normalizeInput(input, options);
    if (!ctx.html && !ctx.doc) return unknownResult('empty');
    if (ctx.html && ctx.html.length > MAX_HTML_SCAN) ctx.html = ctx.html.slice(0, MAX_HTML_SCAN);
    ...
    if (!primary || primary.score < MIN_REPORT_SCORE) {
      ...
      if (fw) {
        return {
          id: 'unknown', name: 'Custom / Unknown', version: null,
          confidence: 'low', score: fw.score, signals: fw.hits,
          related: [{ id: fw.sig.id, name: fw.sig.name }],
          alternates: [], mode: ctx.mode
        };
      }
    ...
    const result = {
      id: primary.sig.id, name: primary.sig.name,
      version: extractVersion(primary.sig, ctx),
      confidence: confidenceFor(primary.score),
      score: primary.score, signals: primary.hits,
      related, alternates, mode: ctx.mode
    };

    if (options.debug) {
      result._debug = scored.map(e => ({ id: e.sig.id, score: e.score, hits: e.hits }));
    }
    return result;
  } catch (_) {
