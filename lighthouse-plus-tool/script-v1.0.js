...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function runAudit(payload) {
    ...
    try {
      const res = await fetch(LH_AUDIT_API, {
        method: 'POST',
      ...
      await saveAuditHistory(auditSaveUrl, 'Lighthouse Plus');
  ...
  function renderReport(data, payload) {
    ...
    results.innerHTML = `
      ...
      <div class="max-w-5xl mx-auto my-16 px-4">
        <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-2xl p-8">
          <h3 class="text-2xl font-bold text-center text-gray-800 dark:text-gray-200 mb-8">Lighthouse Plus Radar</h3>
          <div class="hidden md:block w-full"><canvas id="lh-radar" class="mx-auto w-full max-w-4xl h-[600px]"></canvas></div>
    ...
    setTimeout(() => {
      const canvas = document.getElementById('lh-radar');
        ...
        <div class="mt-8 p-2 md:p-8 bg-white dark:bg-gray-900 rounded-3xl border border-gray-200 dark:border-gray-700 shadow-xl space-y-8">
          <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
            <div class="text-center p-4 rounded-xl bg-gray-50 dark:bg-gray-800">
              ...
              <p class="text-2xl font-black ${lcpColor}">${fmt(bm.lcp)}</p>
              <p class="text-xs mt-1 text-gray-500">ms</p>
            </div>
            <div class="text-center p-4 rounded-xl bg-gray-50 dark:bg-gray-800">
              <p class="text-xs uppercase tracking-wider text-gray-500 mb-1">CLS</p>
              <p class="text-2xl font-black ${clsColor}">${bm.cls.toFixed(3)}</p>
            </div>
            <div class="text-center p-4 rounded-xl bg-gray-50 dark:bg-gray-800">
              <p class="text-xs uppercase tracking-wider text-gray-500 mb-1">FCP</p>
              <p class="text-2xl font-black ${fcpColor}">${fmt(bm.fcp)}</p>
              <p class="text-xs mt-1 text-gray-500">ms</p>
            </div>
            <div class="text-center p-4 rounded-xl bg-gray-50 dark:bg-gray-800">
              <p class="text-xs uppercase tracking-wider text-gray-500 mb-1">TTFB</p>
              <p class="text-2xl font-black ${ttfbColor}">${fmt(bm.ttfb)}</p>
              <p class="text-xs mt-1 text-gray-500">ms</p>
            </div>
            <div class="text-center p-4 rounded-xl bg-gray-50 dark:bg-gray-800">
              <p class="text-xs uppercase tracking-wider text-gray-500 mb-1">TBT</p>
              <p class="text-2xl font-black ${tbtColor}">${fmt(bm.tbt)}</p>
              <p class="text-xs mt-1 text-gray-500">ms</p>
            </div>
          </div>
          ...
          <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
            ...
            <div class="text-center p-3 rounded-xl bg-gray-50 dark:bg-gray-800">
              <p class="text-xs uppercase tracking-wider text-gray-500">Total Transfer</p>
              <p class="text-2xl font-bold text-gray-800 dark:text-gray-200">${fmt((bm.totalTransferSize || 0) / 1024)} KB</p>
            </div>
          </div>
