    ...
    try {
      ...
      results.innerHTML = `
        <div class="max-w-5xl mx-auto space-y-12">
          ...
          <!-- Intent with colored What/How/Why -->
          <div class="text-center mb-12">
            <p class="text-3xl font-bold mb-6">Intent: <span class="bg-gradient-to-r from-orange-400 to-pink-600 bg-clip-text text-transparent">${intent}</span> <span class="text-gray-500">(${confidence}% match)</span></p>
            <div class="max-w-2xl mx-auto p-6 bg-white/10 dark:bg-gray-900/60 rounded-2xl border border-gray-200/20 space-y-6">
              <div><span class="text-blue-500 font-bold text-xl">What it is?</span><p class="mt-2">${intent === 'Informational' ? 'User wants to learn or understand something.' : intent === 'Commercial' ? 'User is researching before buying.' : intent === 'Local' ? 'User is looking for a local business.' : 'User is ready to buy or convert.'}</p></div>
              <div><span class="text-green-500 font-bold text-xl">How to Fix:</span><p class="mt-2">${intent === 'Informational' ? 'Clear headings, lists, images, FAQs.' : intent === 'Commercial' ? 'Comparisons, reviews, pricing tables.' : intent === 'Local' ? 'Map, address, hours, reviews.' : 'Strong CTAs, trust badges.'}</p></div>
            ...
            ${Object.entries(eeat).map(([key, val], i) => `
              <div class="text-center p-6 bg-white dark:bg-gray-900 rounded-2xl shadow-lg border ${val >= 80 ? 'border-green-500' : val >= 60 ? 'border-yellow-500' : 'border-red-500'}">
                ...
                <div class="hidden mt-6 space-y-3 text-left text-sm">
                  <p class="text-blue-500 font-bold">What it is?</p><p>${key === 'Experience' ? 'First-hand knowledge shown.' : key === 'Expertise' ? 'Proven skill/knowledge.' : key === 'Authoritativeness' ? 'Recognized authority.' : 'Site reliability.'}</p>
                  <p class="text-green-500 font-bold mt-3">How to Fix:</p><p>${key === 'Experience' ? '"I/we" stories, photos.' : key === 'Expertise' ? 'Author bio + credentials.' : key === 'Authoritativeness' ? 'Backlinks, schema.' : 'HTTPS, contact page.'}</p>
                  <p class="text-orange-500 font-bold mt-3">Why it Matters?</p><p>${key === 'Experience' ? 'Builds connection.' : key === 'Expertise' ? 'Google favors experts.' : key === 'Authoritativeness' ? 'Higher rankings.' : 'Increases trust.'}</p>
                </div>
              </div>
            `).join('')}
          </div>
          ...
          <div class="text-center mt-16 p-12 bg-gradient-to-r from-orange-500 to-pink-600 text-white rounded-3xl shadow-2xl">
            ...
            <div class="hidden mt-8 space-y-4 text-left max-w-xl mx-auto text-lg">
              <p class="text-blue-400 font-bold">What:</p><p>Estimated SERP position</p>
              <p class="text-green-400 font-bold mt-4">How:</p><p>Apply fixes → monitor GSC</p>
              <p class="text-orange-400 font-bold mt-4">Why:</p><p>Strong E-E-A-T = more traffic</p>
            </div>
          </div>
