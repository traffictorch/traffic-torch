...
data.urlSchema = {
  ...
  compSchema
      ...
      if (data.urlSchema.yourUrlMatch === 0 || !yourSchema) failedModules.push({ id: 'urlSchema', name: 'URL & Schema' });
      const topFixes = [];
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-8 my-12 px-4 max-w-5xl mx-auto">
  ...
  <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-2xl p-8 md:p-10 max-w-md w-full mx-auto border-4 ${yourScore >= 80 ? 'border-green-500' : yourScore >= 60 ? 'border-orange-400' : 'border-red-500'}">
    ...
    ${(() => {
      const title = (yourTitle || '').trim();
      if (!title) return '';
      const truncated = title.length > 65 ? title.substring(0, 65) : title;
      return `<p id="analyzed-page-title" class="mt-6 md:mt-8 text-base md:text-lg text-gray-600 dark:text-gray-200 text-center px-4 leading-tight">${truncated}</p>`;
    })()}
    ...
    <div class="relative w-56 h-56 mx-auto md:w-64 md:h-64">
      <svg viewBox="0 0 200 200" class="w-full h-full transform -rotate-90">
        ...
        <circle cx="100" cy="100" r="90"
                ...
                stroke-width="16" fill="none"
                stroke-dasharray="${(compScore / 100) * 565} 565"
                stroke-linecap="round"/>
      </svg>
      <div class="absolute inset-0 flex items-center justify-center">
        <div class="text-center">
  ...
  ${finalFixes.length > 0 ? `
    <div class="space-y-12">
      ${finalFixes.map((fix, i) => `
        <div class="bg-white dark:bg-gray-900 rounded-3xl shadow-2xl p-8 border-l-8 ${fix.isWorst ? 'border-red-600' : 'border-orange-500'} flex gap-8">
