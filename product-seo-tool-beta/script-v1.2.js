...
const AI_BASE = 'https://product-seo.traffictorch.workers.dev/';
  ...
  //if (!canProceed) return;
    ...
    async function performAnalysis() {
      try {
          ...
          <div class="flex justify-center my-8 sm:my-12 px-0 sm:px-6">
            <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-2xl p-2 sm:p-8 md:p-10 w-full max-w-sm sm:max-w-md border-4 ${safeScore >= 85 ? 'border-emerald-500' : safeScore >= 70 ? 'border-teal-500' : safeScore >= 50 ? 'border-orange-500' : 'border-red-500'}">
              ...
              ${(() => {
                ...
                return truncated ? `<p id="analyzed-page-title" class="mt-6 text-base sm:text-lg text-gray-600 dark:text-gray-200 text-center px-3 sm:px-4 leading-tight">${truncated}</p>` : '';
              })()}
        ...
        // Call the product intent analyzer (must be after DOM parse and before building HTML)
        let aiModuleHTML = '';
        try {
          const cleanedText = seoData.fullText || doc.body.textContent.substring(0, 12000);
          aiModuleHTML = await analyzeProductIntent(cleanedText, url);
          console.log('[Product Intent] Module rendered successfully');
        } catch (intentErr) {
          console.error('[Product Intent] Render failed:', intentErr);
          aiModuleHTML = `<div class="max-w-5xl mx-auto my-16 px-4">
            <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-2xl p-8 text-center border border-red-300 dark:border-red-800">
              <p class="text-red-600 dark:text-red-400">Product intent analysis temporarily unavailable.</p>
            </div>
          </div>`;
        }
  ...
  <div id="share-form-container" class="hidden max-w-2xl mx-auto mt-8">
    <form id="share-form" class="space-y-6 bg-white dark:bg-gray-800 p-8 rounded-3xl shadow-xl border border-orange-500/30">
      ...
      <div>
        ...
        <input id="share-title" type="text" required placeholder="Traffic Torch Product SEO Report" class="w-full bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-2xl px-6 py-4 focus:outline-none focus:border-orange-500">
      </div>
      <div>
  ...
  <div id="feedback-form-container" class="hidden max-w-2xl mx-auto mt-8">
    <div class="bg-white dark:bg-gray-800 p-8 rounded-3xl shadow-xl border border-blue-500/30">
      <p class="text-lg font-medium mb-6 text-gray-800 dark:text-gray-200">
        Feedback for Product SEO Tool on <strong>${document.body.getAttribute('data-url') || 'the analyzed page'}</strong>
      </p>
