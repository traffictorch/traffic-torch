...
// Native <dialog>-based "Show the code" modal.
// Maps audit failure text → CSS selectors, extracts matching snippets
// from the audited HTML, and displays them in a top-layer modal that is
// immune to `contain: paint` / `position: fixed` trapping.
...
const RULES = [
  // ────────────────────────────────────────────────────────────────
  // HTML / performance audit rules (original set)
  // ────────────────────────────────────────────────────────────────
  ...
  // ────────────────────────────────────────────────────────────────
  // SEO Entity Tool rules — match failures produced by
  // coverage.js / salience.js / relationships.js / practices.js / readiness.js
  // (appended after the HTML-audit set so first-match still favours those)
  // ────────────────────────────────────────────────────────────────

  // (a) No entities at all — show the body so the user sees what was scanned
  { test: /No named entities (were )?found/i,
    selectors: ['body'], limit: 1, maxLen: HEAD_MAX,
    note: 'No named entities were detected on this page. Showing the scanned body.' },

  // (b) "used in headings" — strongest actionable signal for entity placement
  { test: /used in headings|Place your top entities/i,
    selectors: ['h1', 'h2', 'h3', 'h4', 'h5', 'h6'], limit: 12,
    note: 'These are the headings on the page. Entities here get the strongest topical weight.' },

  // (c) Schema readiness / schema candidates
  { test: /schema readiness|JSON-LD|structured data|schema candidates|lacks strong schema/i,
    selectors: ['script[type="application/ld+json"]'],
    note: 'No JSON-LD script tags found in the page. Add structured data in <head> for your top entity types.' },

  // (d) Name consistency
  { test: /Inconsistent entity naming|Name consistency/i,
    selectors: ['title', 'h1', 'h2', 'img[alt]'], limit: 10,
  ...
  { test: /weak prominence|prominence is low|salience distribution|too flat|primary entity/i,
    selectors: ['title', 'meta[name="description"]', 'h1'], limit: 3,
    note: 'The prominent positions where your primary entity carries the most weight.' },

  // (g) Bold / strong / em usage
  { test: /strongly emphasized entities|bold tags/i,
    selectors: ['strong', 'b', 'em'], limit: 10,
    note: 'Existing emphasized text — key entities belong in tags like these.' },

  // (h) Entity count low / too few / very few
  { test: /Entity count is quite low|Too few entities|Very few entities|fewer than 4 entities/i,
    selectors: ['h1, h2, h3', 'p'], limit: 8,
    note: 'Headings and paragraphs where additional entities can be woven in naturally.' },

  // (i) Entity density (high or low) — show paragraphs
  { test: /Entity density|keyword stuffing/i,
    selectors: ['p'], limit: 10,
    note: 'Paragraphs where entity density is measured.' },

  // (j) Type diversity / synergy / narrow range
  { test: /type diversity|type synergy|Narrow range of entity types|Weighted topical diversity/i,
    selectors: ['h1, h2, h3', 'p'], limit: 8,
    note: 'Where new complementary entity types can be introduced.' },

  // (k) Co-occurrence / topical clustering
  { test: /co-occurrence|co-mention|topical clustering|clusters?/i,
    selectors: ['h1, h2, h3', 'p'], limit: 8,
...
// ────────────────────────────────────────────────────────────────
// Internal: cached DOMParser instance
// ────────────────────────────────────────────────────────────────
let _cachedDoc = null;
...
// ────────────────────────────────────────────────────────────────
// Internal: raw-text search (for deprecated-API style rules)
// ────────────────────────────────────────────────────────────────
...
// ────────────────────────────────────────────────────────────────
// Modal — native <dialog> (always centers, escapes any CSS trap)
// ────────────────────────────────────────────────────────────────
...
// ────────────────────────────────────────────────────────────────
// Public: one-shot helper used by the click handler
// ────────────────────────────────────────────────────────────────
