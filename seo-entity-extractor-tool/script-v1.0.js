document.addEventListener('DOMContentLoaded', () => {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!urlInput) {
      console.error('Input #url-input not found');
    ...
    try {
      ...
      // Render all extracted entities (no limit)
      const entitiesHTML = data.extracted?.map(entity => `
        <div class="p-4 bg-white dark:bg-gray-900 rounded-xl shadow-md border border-gray-200 dark:border-gray-700">
          ...
          <p class="text-xs mt-1 text-gray-500 dark:text-gray-400">
            Salience: ${entity.salience.toFixed(2)} (how central this entity is)
          </p>
        </div>
      `).join('') || '<p class="text-gray-600 dark:text-gray-400">No entities detected.</p>';

      // Simple audit summary
      const audit = data.audit || { overall: 0, suggestions: [] };
      const grade = getGrade(audit.overall);
      const suggestionsHTML = audit.suggestions?.map(s => `<li class="text-gray-800 dark:text-gray-200">${s}</li>`).join('') || '<li>No suggestions available.</li>';

      results.innerHTML = `
        <div class="max-w-4xl mx-auto px-4 py-8">
          <div class="text-center mb-12">
            <h2 class="text-3xl font-bold text-gray-800 dark:text-gray-200">Entity Analysis Report</h2>
            <p class="mt-2 text-lg text-gray-600 dark:text-gray-400">Overall Semantic Health: <span class="$$   {grade.color} font-bold">   $${audit.overall}/100 ${grade.emoji} ${grade.text}</span></p>
          </div>
      ...
      // Re-init share/save/feedback if you have those functions
