// vite.config.js
import { defineConfig } from 'vite'
import { resolve } from 'path'

export default defineConfig({
  base: '/',
  build: {
    rollupOptions: {
      input: {
        main: resolve(__dirname, 'index.html'), // root homepage / shell
        // 'seo-ux-tool': resolve(__dirname, 'seo-ux-tool/index.html'), // ← corrected
        'local-seo-tool': resolve(__dirname, 'local-seo-tool/index.html'),
        'topical-authority-audit-tool': resolve(__dirname, 'topical-authority-audit-tool/index.html'),
        'seo-entity-extractor-tool': resolve(__dirname, 'seo-entity-extractor-tool/index.html'),
        'schema-generator': resolve(__dirname, 'schema-generator/index.html'),
        'seo-intent-tool': resolve(__dirname, 'seo-intent-tool/index.html'),
        'product-seo-tool': resolve(__dirname, 'product-seo-tool/index.html'),
        'quit-risk-tool': resolve(__dirname, 'quit-risk-tool/index.html'),
        'ai-voice-search-tool': resolve(__dirname, 'ai-voice-search-tool/index.html'),
        'ai-audit-tool': resolve(__dirname, 'ai-audit-tool/index.html'),
        'ai-search-optimization-tool': resolve(__dirname, 'ai-search-optimization-tool/index.html'),
        'keyword-tool': resolve(__dirname, 'keyword-tool/index.html'),
        'keyword-vs-tool': resolve(__dirname, 'keyword-vs-tool/index.html'),
        'keyword-research-tool': resolve(__dirname, 'keyword-research-tool/index.html'),
        'about': resolve(__dirname, 'about/index.html'),
        'community': resolve(__dirname, 'community/index.html'),
        'ai-seo-ux-tools': resolve(__dirname, 'ai-seo-ux-tools/index.html'),
        'contact': resolve(__dirname, 'contact/index.html'),
        'pro': resolve(__dirname, 'pro/index.html'),
        'privacy': resolve(__dirname, 'privacy/index.html'),
        'terms': resolve(__dirname, 'terms/index.html'),
        'news': resolve(__dirname, 'news/index.html'),
        '404': resolve(__dirname, '404.html')
        // Blog posts are static → handled by Cloudflare Pages copy step (no need in input)
      }
    }
  }
})