...
// product-torch/script.js - full complete epic perfect version with realistic metrics
...
// Helper functions for realistic product SEO metric calculation
...
// Returns array of all Product schema objects found (or empty array)
function extractProductSchema(doc) {
  ...
  scripts.forEach(script => {
    try {
      ...
      // Silent fail — bad JSON
    }
  });
  return schemas;
}

// Returns true if a likely review/rating section is present (common eCom patterns)
function hasReviewSection(doc) {
  const selectors = [
    '.reviews', '.product-reviews', '#reviews', '.rating', '.yotpo', '.judge-me',
    '[data-reviews]', '.aggregate-rating', '.star-rating', '.customer-reviews',
    '[itemprop="aggregateRating"]', '[itemprop="review"]'
  ];
  return selectors.some(sel => doc.querySelector(sel));
}

// Returns true if price/availability markup is likely present in schema or visible text
function hasPriceMarkup(doc, schema) {
  if (schema && schema.offers && (schema.offers.price || schema.offers.availability)) return true;
  // Fallback: look for common price/stock patterns
  const pricePatterns = /\$[0-9,.]+|USD [0-9,.]+|€ [0-9,.]+|£ [0-9,.]+|In Stock|Out of Stock/i;
  return pricePatterns.test(doc.body.textContent);
}

// Returns true if Open Graph meta tags for social sharing are present
function hasSocialMeta(doc) {
  return !!doc.querySelector('meta[property^="og:"]');
}

// Cleaned and simplified UX content extraction – now focused on product page essentials
...
// Main product SEO analyzer – replaces analyzeUX
function analyzeProductSEO(doc, inputUrl) {
  const data = getProductPageContent(doc, inputUrl);

  // Category analyzers (to be implemented next – placeholders for now)
  ...
  const ecommerceResult = analyzeEcommerceSEO(doc, data); // Pro category

  // Weighted scoring (adjust percentages later if needed)
  const weights = {
    onPage: 0.30,
    technical: 0.25,
    contentMedia: 0.25,
...
// Placeholder analyzer skeletons – will be filled in next steps
function analyzeOnPageSEO(doc, data) {
  // To be implemented: title, meta, headings, URL, keywords
  return { score: 70, details: {} }; // Placeholder
}

function analyzeTechnicalSEO(doc, data) {
  // To be implemented: viewport, HTTPS, canonical, robots, sitemap hint
  return { score: 85, details: {} }; // Placeholder
}

function analyzeContentMedia(doc, data) {
  // To be implemented: description quality, images, video, UGC, links, breadcrumbs
  return { score: 65, details: {} }; // Placeholder
}

function analyzeEcommerceSEO(doc, data) {
  // To be implemented: schema, price/availability, reviews, variants, social meta (Pro)
  return { score: 50, details: {}, isPro: true }; // Placeholder + pro flag
}

// Health status label (replaces Quit Risk)
...
// Grade info for overall & category scores
...
// Plugin/solution grade helper (unchanged logic, reused for SEO plugins)
...
// Updated module card builder – now works with new factorDefinitions
function buildModuleHTML(moduleName, value, moduleData, factorScores = null) {
  ...
  moduleData.factors.forEach(f => {
    ...
    // Remove old module-specific passed checks (Readability/Navigation/etc.)
    // New categories use simple threshold comparison (already done above)
