export function initShareReport(resultsContainer) {
  ...
  shareBtn.addEventListener('click', async () => {
    ...
    pageTitle = pageTitle.replace(/Product SEO: eCommerce Product Page Auditor | Traffic Torch/gi, '').trim() || 'this page';

    const shareText = `Check out ${pageTitle} on Traffic Torch Product SEO Tool ${shareUrl}`;

    try {
      if (navigator.share) {
        await navigator.share({
          title: 'Traffic Torch Product SEO Report',
          text: shareText
        });
