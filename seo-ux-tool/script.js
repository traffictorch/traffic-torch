document.addEventListener('DOMContentLoaded', () => {
    ...
    issues.forEach(i => {
      ...
      li.innerHTML = `
        <div class="p-5 bg-white/10 backdrop-blur rounded-2xl mb-4 border border-white/20">
          ...
          <p class="mb-3"><span style="color:#60a5fa;font-weight:700">What it is?</span><br>${i.what || 'A common SEO or UX issue that affects how users and search engines see your page.'}</p>
          <p class="mb-3"><span style="color:#34d399;font-weight:700">How to Fix:</span><br>${i.fix}</p>
          <p><span style="color:#f87171;font-weight:700">Why it Matters?</span><br><strong>UX:</strong> ${i.uxWhy || 'Improves user experience, navigation, and accessibility'}<br><strong>SEO:</strong> ${i.seoWhy || 'Helps Google understand and rank your page better'}</p>
        </div>
      `;
      ul.appendChild(li);
    });
  ...
  function analyzePerf(html, doc) {
    ...
    const weightIssue = sizeKB > 300 ? 'Very heavy' : sizeKB > 150 ? 'Heavy' : 'Good';
    if (sizeKB > 150) {
      score -= sizeKB > 300 ? 30 : 15;
      issues.push({
        issue: `Page weight: ${sizeKB} KB (${weightIssue})`,
