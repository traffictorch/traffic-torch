...
// Improved questions: count only ? in sentence-like structures (exclude titles/headings)
...
// Pain points - require proximity to reader pronouns for stronger relevance
...
const hasInterviews = /\b(interview(?:ed|s)?|spoke with|talked to|surveyed|asked|quoted|said|reports|says|according to|told me|told us|shared with me|shared with us)\b/i.test(mainText) &&
                     /["“][^"]{20,}["”]/.test(mainText);  // also requires a reasonably long quote

let uniqueInsights = 0;

// Depth – tiered instead of hard cut-off
...
if (hasInterviews) uniqueInsights += 15;
      ...
      const tests = [
		...
		{ emoji: painPointsNearYou > 2 ? '✅' : '❌', text: 'Reader pain points acknowledged', passed: painPointsNearYou > 2 },
